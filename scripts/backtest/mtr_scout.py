#!/usr/bin/env python3
"""
MTR Scout — Macro Trend Rider
==============================
Hypothese: 4H-Breakouts über ein 20-Tage-Makro-Level (120 × 4H-Bars) erfordern
institutionelles Kapital und können nicht wie 1H-Setups billig gefaked werden.
Chandelier Trailing Stop fängt die resultierenden Makro-Trends komplett ein.

Signal-Logik (4H):
  LONG:  Close > Highest(High, 120 Bars)  +  Vol > 1.2×SMA(50)
         [120 × 4H = 20 Tage Makro-Level]
  SHORT: Close < Lowest(Low, 120 Bars)    +  Vol > 1.2×SMA(50)
...
Unterschied zu ATR-Rider: 4H-Timeframe statt 1H, 120-Bar-Lookback (20 Tage)
statt 48H-Donchian. Hypothese: Makro-Level haben höhere Follow-Through-Rate.

Usage:
  venv/bin/python3 scripts/backtest/mtr_scout.py
  venv/bin/python3 scripts/backtest/mtr_scout.py --assets ETH,BTC --dir long
  venv/bin/python3 scripts/backtest/mtr_scout.py --from 2026-01-01
"""
import argparse
import math
import os
...
# ─── Parameter ────────────────────────────────────────────────────────────────
LOOKBACK   = 60   # 120 × 4H = 20 Tage Makro-Level
ATR_PERIOD = 14
...
def kpis(r_list: list[float]) -> dict:
    ...
    if n > 1:
        ...
        # Sharpe annualisiert auf 4H-Basis: 365d × 6 Bars/Tag = 2190 Bars/Jahr
...
def run_mtr(candles: list[dict], direction: str = "both") -> dict:
    """Backtestet MTR auf 4H-Candles."""
...
def print_report(asset: str, result: dict) -> None:
    for side, r_list in [("LONG", result["long"]), ("SHORT", result["short"])]:
        ...
        print(f"  TotalR={k['total_r']:+.1f}R  MaxDD={k['max_dd']:.2f}R  "
              f"Sharpe(ann,4H)={k['sharpe']:.2f}  t={k['t']:.3f}")
...
def main() -> None:
    ...
    print(f"MTR Scout — 4H × {LOOKBACK} Bars = 20-Tage-Makro-Level")
    print(f"Chandelier: {ATR_MULT}×ATR({ATR_PERIOD})  |  Vol-Filter: >{VOL_MULT}×SMA({VOL_PERIOD})")
    ...
    for asset in assets:
        ...
        result = run_mtr(candles_4h, direction=args.dir)
